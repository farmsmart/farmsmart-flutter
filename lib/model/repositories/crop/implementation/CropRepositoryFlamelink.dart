...
CropEntity _transform(FlameLink cms, DocumentSnapshot snapshot) {
  final transformer = FlamelinkCropTransformer(cms: cms, metaTransformer: FlamelinkMetaTransformer());
...
class CropEntityCollectionFlamelink implements EntityCollection<CropEntity> {
  ...
  CropEntityCollectionFlamelink({FlamelinkDocumentCollection collection, bool onlyPublished = true})
   ...
   switch (group) {
      default:
        final publishedDocuments = _cms
            .documentsQuery(schema: _Fields.cropSchema, limit: limit)
