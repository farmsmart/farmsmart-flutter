...
class MockListRepository<T> implements BasicRepositoryInterface<T> {
  ...
  MockListRepository({IdentifyEntity<T> identifyEntity, List<T> startingData = const []}): this._identFunction = identifyEntity, this._objects = startingData;   
  ...
  Future<T> getSingle(String uri) {
    final entity = _objects.singleWhere((entity) => _identFunction(entity) == uri, orElse: () => null);
